use { eframe::egui::{Ui,RichText, Color32,FontId, TextEdit }};
...
impl CashWidget {
    ...
    pub fn show(&mut self, ui: &mut Ui) {
        ui.columns_const(|[ui_1, ui_2, ui_3]| {
            ...
            if !self.cash_highlighted {
                cash.state.cursor.set_char_range(Some(CCursorRange::two(CCursor::new(0),CCursor::new(self.cash_text.len()))));
    ...
    fn create_total_dollar_label(&mut self, ui: &mut Ui) {
        ui.label(
            RichText::new(" = $".to_string() + &(format!("{:.2}",self.get_total_currency_dollar()).to_string()))
                .monospace()
                .size(17.5)
                .color(Color32::WHITE),
        );
